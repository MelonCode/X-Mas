...
/**
 * Class to display particles using Reflection, including 1.8 particles. </br>
 * You are NOT allowed to claim this as your own.
 *
 * @author inventivetalent - {@link http://www.inventivegames.de}
 */
public enum ParticleEffect {
    ...
    /**
     * Modify the maximum Range of particles (only useful for Client versions
     * 1.8+)
     *
    ...
    private static boolean isPlayerInRange(Player p, Location center) {
        double distance;
        return !((distance = center.distance(p.getLocation())) > Double.MAX_VALUE) && distance < particleRange;
    }
    ...
    public void sendToPlayer(Player player, Location location, float offsetX, float offsetY, float offsetZ, float speed, int count) throws Exception {
        ...
        if (ReflectionUtilities.getVersion().contains("v1_8")) {
            try {
                ...
                Object packet = nmsPacketPlayOutParticle.getConstructor(new Class[]{nmsEnumParticle, boolean.class, float.class, float.class, float.class, float.class, float.class, float.class, float.class, int.class, int[].class}).newInstance(getEnum(nmsEnumParticle.getName() + "." + (enumValue != null ? enumValue : name().toUpperCase())), true, (float) location.getX(), (float) location.getY(), (float) location.getZ(), offsetX, offsetY, offsetZ, speed, count, new int[]{});
    ...
    /**
     * Send the particle to a Collection of Players
     * {@link #sendToPlayer(Player, Location, float, float, float, float, int)}
     *
     * @param player   Receiver of the particle
     * @param location Location of the particle
     * @param offsetX  X size of the area to spawn particles in.
    ...
    /**
     * Send the particle to an Array of Players
     * {@link #sendToPlayer(Player, Location, float, float, float, float, int)}
     *
     * @param player   Receiver of the particle
     * @param location Location of the particle
